...
"modalId"=>"add".$modalName,
"modalRoute"=>($modalRoute??$modalName).".store",
...
<h5 class="modal-title text-white" id="{{$modalName}}Label">{{ trans('translation.add-new-'.$modalName) }}
</h5>
...
<div class="hstack gap-2 justify-content-end">
    ...
    <button type="submit" class="btn btn-primary" id="submit-{{$modalName}}-btn" disabled>{{ trans('translation.add') }}</button>
</div>
@endslot
...
@push('after-scripts')
    <script>
        $(document).ready(function(){
            $('#add' + '{{$modalName}}' + ' .check-empty-input').on('change', function(){
                let flag = true;
                ...
                $('#add' + '{{$modalName}}' + ' .check-empty-input').each(function() {
                    if (!$(this).is('div')){
