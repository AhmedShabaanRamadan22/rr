    ...
    <div class="modal-dialog modal-lg modal-dialog-centered ">
        <div class="modal-content ">
            ...
            <div class="modal-header">
                <h6 class="modal-title">{{trans('translation.order-interview-notes')}}</h6>
        ...
        $('#update_notes').click(function() {
            ...
            $.ajax({
                ...
                success: function(response, jqXHR, xhr) {
                    ...
                    if(notes == "" || notes == "<br>"){
                        $('.notes-button[data-order-id="'+order_id+'"]').removeClass('btn-outline-primary').addClass('btn-outline-secondary')
                    }else{
                        $('.notes-button[data-order-id="'+order_id+'"]').removeClass('btn-outline-secondary').addClass('btn-outline-primary')
                    }
                    if (xhr.status === 200) {}
                }
            });
        ...
        function getOrderNote(order_id) {
            $.ajax({
                ...
                success: function(response) {
                    $('#notes').summernote('code',response.interview_note, {
                        height: 200
                    });
