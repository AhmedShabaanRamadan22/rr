            ...
            <div class="card-body">
                <div class="tab-content text-muted">
                    <div class="tab-pane show active" id="custom-hover-services">
                            ...
                            <div class="tab-content text-muted">
                                ...
                                <div class="tab-pane {{$loop->index == 0 ? 'active' : ''}}" id="border-nav-{{$service->id}}" role="tabpanel">
                                    @component('admin.organizations.settings.contract-template', ['organization'=>$organization, 'name' => $service->service->name, 'template_type' => 'service_' . $service->id, 'dictionaries'=>$order_sector_dictionaries])@endcomponent
                                </div>
                                @endforeach
                            </div>
                    </div>
                    ...
                    <div class="tab-pane" id="custom-hover-employees">
                        @component('admin.organizations.settings.contract-template', ['organization'=>$organization, 'name' => trans('translation.employees'), 'template_type' => 'employee_' . $organization->id, 'dictionaries'=>$user_dictionaries])@endcomponent
                    </div>
                </div>
            </div><!-- end card-body -->
        </div>
