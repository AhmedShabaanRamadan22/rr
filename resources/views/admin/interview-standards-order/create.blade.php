...
<div class="card">
    ...
    <div class="card-body">
        <!-- <div class="tab-content"> -->
            ...
            <form id="addNewInterview" action="{{route('order-interviews.store')}}" method="POST">
                @csrf
                <input type="hidden" name="order_id" value="{{$order->id}}">
                @forelse ($interview_standards as $interview_standard)
                <input type="hidden" name="max_scores[{{$interview_standard->id}}]" value="{{$interview_standard->max_score}}">
                <div class="row mb-3">
                    <div class="col-lg-4">
                        <label for="input-{{$interview_standard->id}}" class="form-label">{{$interview_standard->name ?? '-'}}
                            <span class="text-danger">*</span>
                        </label>
                        <span class="text-muted fs-xs mb-0 mx-2 ">{{trans('translation.max-score')." ".$interview_standard->max_score}} </span>
                        <p class="text-muted " style="font-size: 12px;">{!! $interview_standard->description !!}</p>
                    </div>
                    <div class="col-lg-8">
                        <select class="form-select" id="input-{{$interview_standard->id}}" name="scores[{{$interview_standard->id}}]" required>
                            @php
                                $score_suggestion = $interview_standard->get_score_suggestion($order);
                            @endphp
                            @for ($i = 0; $i <= $interview_standard->max_score; $i++)
                                <option value="{{$i}}" {{$i == $score_suggestion ? 'selected':''}} >{{$i}}</option>
                                @endfor
                        </select>
                    </div>
                ...
                <!-- <div class="row mb-3">
                    ...
                    <div class="col-lg-9">
                        <input type="number" class="form-select" id="input-interview-bonus" min="0" name="bonus" placeholder="{{trans('translation.interview-bonus')}}">
                    </div>
                </div> -->
                <div class="text-end">
