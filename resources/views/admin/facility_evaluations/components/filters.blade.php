<div class="row">
    ...
    <div class="col-lg-3 col-sm-12 my-auto">
        <button class="btn btn-primary" id="{{$tableName}}-filter-btn">{{trans('translation.filter')}}</button>
        <button class="btn btn-secondary" id="{{$tableName}}-reset-btn">{{trans('translation.reset')}}</button>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        $('#{{$tableName}}-filter-btn').click(function() {
            {{ 'window.' . $tableName . 'Datatable' }}.ajax.reload();
        });
        
        $('#{{$tableName}}-reset-btn').click(function() {
