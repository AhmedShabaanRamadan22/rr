...
class CategoryController extends Controller
{
    ...
    public function dataTable(Request $request)
    {
        ...
        return datatables($query)
            ->addColumn('action', function ($row) {
                ...
                <button
                class="btn btn-outline-danger btn-sm m-1  on-default m-r-5 deletecategories" data-model-id="' . $row->id . '">
    ...
    {
        if ($delete_model->organization_categories->isNotEmpty()) {
            return trans('translation.delete-organization_categories-first');
        }
        return '';
    }
}
